...
# Get the pod name
POD_NAME=$(kubectl get pods -n $NAMESPACE --no-headers -o custom-columns=":metadata.name" | grep "^test-deployment")
...
if [[ -n "$POD_NAME" ]]; then
    ...
    echo "No pod found matching test-deployment pattern."
    exit 1
fi
